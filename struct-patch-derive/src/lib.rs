...
pub fn derive_patch(item: TokenStream) -> TokenStream {
    ...
    let wrappered_fields = &mut fields_with_type
    .iter()
    .map(|(f, t)| {
        (
            f.clone(),
    ...
    let field_names = wrappered_fields.iter().map(|(f, _)| f);
    let field_names_clone = field_names.clone();
    let wrappered_types = wrappered_fields.iter().map(|(_, t)| t);

    let mut output = if let Some(patch_derive) = patch_derive {
        quote!(
            #[derive #patch_derive]
            pub struct #patch_struct_name {
                #(pub #field_names: #wrappered_types,)*
            }
        )
    } else {
        quote::quote!(
